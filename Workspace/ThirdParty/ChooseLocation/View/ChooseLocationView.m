...
- (void)setUp{
    ...
    cancelBtn.frame = CGRectMake(topView.width-60, 0, 60, topView.height);
...
- (void)setUpAddress:(AddressItem *)address{

    NSInteger index = self.contentView.contentOffset.x / HYScreenW;
    UIButton * btn = self.topTabbarItems[index];
    [btn setTitle:address.name forState:UIControlStateNormal];
    btn.tag = address.code.intValue;
    [btn sizeToFit];
    [_topTabbar layoutIfNeeded];
    [self changeUnderLineFrame:btn];
...
//        }
        ...
        [codesStr appendString:[NSString stringWithFormat:@"%zd", btn.tag]];
        [codesStr appendString:@","];
    }
    if (addressStr.length>0) {
        [addressStr deleteCharactersInRange:NSMakeRange(addressStr.length-1, 1)];
    }
    if (codesStr.length>0) {
        [codesStr deleteCharactersInRange:NSMakeRange(codesStr.length-1, 1)];
    }
    self.address = addressStr;
    self.addressCode = codesStr;
...
- (void)scrollToNextItem:(AddressItem *)address{
